    ...
    viz = new $jit.ForceDirected({
        ...
        Tips: {
            ...
            onShow: function (tip, node) {
                ...
                if (pluginJSON) {
                    html +=
                        '<div class="tip-text" style=\'margin-bottom: 5px;\'><b>Excerpt:</b> ' +
                        pluginJSON.excerpt +
                        '</div>'
                    ...
                    if (pluginJSON.dependencies.length > 0) {
                        ...
                        for (var i = 0; i < pluginJSON.dependencies.length; i++) {
                            ...
                            html +=
                                ' (version: ' +
                                pluginJSON.dependencies[i].version +
                                ' / optional: ' +
                                pluginJSON.dependencies[i].optional +
                                ')'
                            html += '</li>'
                        }
                        html += '</ul></div>'
...
function getJsonForChart(filter) {
    ...
    for (var plugin in json.plugins) {
        ...
        if (
            !filterRegexp ||
            pluginJSON.name.match(filterRegexp) ||
            pluginJSON.title.match(filterRegexp) ||
            containsDeveloper(pluginJSON, filterRegexp)
        ) {
            var pluginNode = {
                id: pluginJSON.name,
